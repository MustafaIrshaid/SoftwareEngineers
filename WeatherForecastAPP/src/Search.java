...
public class Search {
    ...
    public static ArrayList<User> searchEmployees(String name){
        Connection con=null;
        ArrayList<User> matchedUsers = new ArrayList<>();
        try{
             Class.forName("com.mysql.jdbc.Driver");
        con= DriverManager.getConnection("jdbc:mysql://localhost:3306/softproj","root","");
        String sq1 = "select * from user where username LIKE '%"+name+"%' and manager_id = '"+ Login.currentUser.getUserID() +"'";
        
        if(name.length() == 0) sq1 = "select * from user where manager_id = '"+ Login.currentUser.getUserID() +"'";
        Statement st = con.createStatement();
        ResultSet rs=st.executeQuery(sq1);
        while(rs.next()){
            matchedUsers.add(new User(rs.getInt(1),rs.getString(2),"",rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getInt(8),rs.getString(9)));
